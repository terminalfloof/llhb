...
function glitch(jQueryElement, message, amountOfSteps, timeBetweenSteps) {
  ...
  if (doDebug){
    console.log(`Doing ${aOS} steps with ${tBS} seconds in between, totalling ${aOS * tBS / 1000} seconds.`)
  }
    ...
    if (i !== (aOS - 1)) {
      setTimeout(() => {
        ...
        for (var j = 0; j < messageLength; j++) {
          glitchMessage += glitchLetters[Math.floor(Math.random() * glitchLetters.length)]
        }
        if (doDebug) {
...
function loadStatus(status, userData) {
  ...
  try {
    ...
    console.log(err);
    app.html(`Looks like the developer of this site sucks at coding. There was an error. <br><br><span style="color: green; font-family: monospace;">${err.message}</span>`)
  }
}
        ...
        $("#inputdiv").fadeOut(500, function () {
          $("#inputdiv").hide(() => {
            ...
            if (data.name === "?????") {
              ...
              let finalString = ""
              for (var i = 0; i < 5; i++) {
                let currentSymbol = randomSymbols[Math.floor(Math.random() * randomSymbols.length)];
                if (currentSymbol === "r") {
              ...
              $("#app").html(`Welcome to the MDBS, <span class="glitch" style="font-size: 1em" data-text="${finalString}">${finalString}.</span>`);
