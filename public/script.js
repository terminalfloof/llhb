...
class DebugTool {
  ...
  get textGlitch() { return this._textGlitch; }

  get timerGlitch() { return this._timerGlitch; }

  openApp() {
    $("form").hide();
    $("#app").show(() => { console.log("app opened."); });
  }
  ...
  stopTimer() { throw new Error("The timer hasn't started yet."); }
}
...
function glitch(jQueryElement, message, amountOfSteps, timeBetweenSteps,
  ...
  if (doDebug) {
    console.log(`Doing ${aOS} steps with ${tBS} seconds in between, totalling ${
        (aOS * tBS) / 1000} seconds.`);
  }
...
var stopTimer = () => { throw new Error("The timer hasn't started yet."); };
// just a countdown
function showEnd(date, endMessage) {
  ...
  function showRemaining() {
    ...
    if (days == 0) {
      glitchHtml =
          `<span class="glitch" style="font-size: 1em" data-text="00.">00.</span>`;
      if (hours == 0) {
        glitchHtml =
            `<span class="glitch" style="font-size: 1em" data-text="00.00">00.00</span>`;
        if (minutes == 0) {
          glitchHtml =
              `<span class="glitch" style="font-size: 1em" data-text="00.00.00">00.00.00</span>`;
          if (seconds == 0) {
            glitchHtml = ``;
          } else {
            html = `${seconds}`;
...
function loadStatus(status, userData) {
  ...
  try {
    ...
    case "countdown":
      ...
      break;
    case "survey":
      if (status.data.users.includes(userData.key) ||
        ...
        $("#survey").submit(e => {
          ...
          if (acceptedValues.includes(
            ...
            socket.emit("surveyReply", userData.key,
                        parseInt(document.forms["survey"]["response"].value));
            $("#app").fadeOut(500, () => {
              $("#app").html(`Thanks for submitting, ${
                  userData
                      .name}.<br><span style="font-size: 0.75em">You submitted <strong>${
                  document.forms["survey"]["response"]
                      .value}</strong></span>.`);
    ...
    console.log(err);
    app.html(
        `Looks like the developer of this site sucks at coding. There was an error. <br><br><span style="color: green; font-family: monospace;">${
            err.message}</span>`);
  }
}
...
$("#idinput").submit(event => {
  ...
  socket.emit(
      "login", parseInt(document.forms["idinput"]["id"].value),
          ...
          $("#inputdiv").fadeOut(500, function() {
            $("#inputdiv").hide(() => {
              ...
              if (data.name === "?????") {
                ...
                let finalString = "";
                for (var i = 0; i < 5; i++) {
                  let currentSymbol = randomSymbols[Math.floor(
                      Math.random() * randomSymbols.length)];
                  if (currentSymbol === "r") {
                ...
                $("#app").html(
                    `Welcome back to the MDBS, <span class="glitch" style="font-size: 1em" data-text="${
